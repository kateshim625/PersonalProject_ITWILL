...
<table id="notice" class="view">
		<tr><td>Num</td><td><%=bb.getNum() %></td>
			<td>ReadCount</td><td><%=bb.getReadCount() %></td></tr>
	    <tr><td>Writer</td><td><%=bb.getWriter() %></td>
			<td>Date</td><td><%=bb.getDate() %></td></tr>
		<tr><td>Title</td><td colspan="3"><%=bb.getTitle() %></td></tr>
		<tr class="con"><td>Contect</td><td colspan="3"><%=bb.getContent() %></td></tr>
</table>
...
if(id!=null){
	if(id.equals(bb.getWriter())){
		%>
		<input type="button" value="Update" class="btn" onclick="location.href='writeUpdateForm.jsp?num=<%=bb.getNum()%>'">
		<input type="button" value="Delete" class="btn" onclick="location.href='writeDeleteForm.jsp?num=<%=bb.getNum()%>'">
		<%
	}
}
%>
